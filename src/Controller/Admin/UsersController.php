...
class UsersController extends AppController
{
	...
	public function index()
	{
		...
		public function delete($id = null) // Gunakan ID jika slug bermasalah
	{
		// Memastikan hanya request berbentuk POST atau DELETE dibenarkan
		$this->request->allowMethod(['post', 'delete']);
		
		// Gantikan findBySlug kepada get($id) jika anda menghantar ID dari View
		// Atau kekalkan findBySlug jika anda pasti slug itu wujud
		try {
			$user = $this->Users->get($id); 
			
			if ($this->Users->delete($user)) {
				$this->Flash->success(__('The user has been successfully deleted.'));
			} else {
				$this->Flash->error(__('The user could not be deleted. Please, try again.'));
			}
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $e) {
			$this->Flash->error(__('The user data not found'));
		}
